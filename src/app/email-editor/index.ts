...
import loadPanels from './utils/panels';
...
const plugin: Plugin<PluginOptions> = (editor, opts: Partial<PluginOptions> = {}) => {
  ...
  const options: RequiredPluginOptions = {
    blocks: ['sect100', 'sect50', 'sect30', 'sect25', 'sect37', 'button', 'divider', 'text', 'text-sect', 'image', 'quote',
      'link', 'link-block', 'grid-items', 'list-items'],
    block: () => ({}),
    juiceOpts: {},
    cmdOpenImport: 'gjs-open-import-template',
    cmdTglImages: 'gjs-toggle-images',
    cmdInlineHtml: 'gjs-get-inlined-html',
    modalTitleImport: 'Importa template',
    modalTitleExport: 'Esporta template',
    modalLabelImport: '',
    modalLabelExport: '',
    ...
    useCustomTheme: true,
    textCleanCanvas: 'Sei sicuro di voler cancellare la newsletter?',
    ...opts,
  };
  ...
  if (options.useCustomTheme && typeof window !== 'undefined') {
    const primaryColor: string = '#E2E8F0';
    const secondaryColor: string = '#64748b';
    const tertiaryColor: string = '#7928ca';
    const quaternaryColor: string = '#ff0080';
    const prefix: string = 'gjs-';
